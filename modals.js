...
window.openModal = function(type, data = {}) {
  ...
  if (type === 'client') {
    ...
    html = `
      ...
      <div class="form-row"><label>Notes</label><textarea id="m-notes"></textarea></div>
      <input type="hidden" id="m-oferta-origen" value="${data._ofertaId||''}">
      <div class="modal-actions">
        <button class="btn" onclick="closeModal()">Cancel·lar</button>
...
window.saveOferta = async function(id) {
  ...
  if (estat === 'Tancada guanyada') {
    // NO esborrem l'oferta aquí (si l'usuari cancel·la el modal de consolidat,
    // la perdria). La passem al modal; saveConsolidat l'arxivarà NOMÉS després
    // d'haver guardat el tancament amb èxit.
    closeModal();
    openModal('consolidat', { ...dades, _ofertaId: id || null, prima_brokkom: dades.prima_brokkom });
    return;
  }
  try {
    if (id) {
...
window.saveConsolidat = async function(id) {
  ...
  try {
    ...
    if (error) throw error;
    // Tancament guardat amb èxit → ara sí, arxivem l'oferta d'origen (si en ve).
    const ofertaId = document.getElementById('m-oferta-origen')?.value;
    if (ofertaId) {
      const { error: errDel } = await supabase.from('ofertes').delete().eq('id', ofertaId);
      if (errDel) console.warn('No s\'ha pogut arxivar l\'oferta origen:', errDel.message);
    }
    await refreshData();
    closeModal();
    showTab('consolidats');
...
window.saveTasca = async function() {
  ...
  const dades = {
    ...
    data_prevista: document.getElementById('m-data-prevista').value || new Date().toISOString().slice(0,10),
