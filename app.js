...
async function loadAllData() {
  ...
  const tables = ['clients','ofertes','consolidats','seguiments','oportunitats','venciments','tasques','asseguradores','posts','inbox_items','notes','agenda_events','esborranys','vinculacions','comparticions','mediadors','prospectes','trucades'];
  const stateKeys = ['clients','ofertes','consolidats','seguiments','oportunitats','venciments','tasques','asseguradores','posts','inbox','notes','agenda','esborranys','vinculacions','comparticions','mediadors','prospectes','trucades'];
...
window.refreshData = async (only, opts = {}) => {
  const map = {
    ...
    prospectes: 'prospectes', trucades: 'trucades'
  };
  if (only && map[only]) {
    try {
...
window.updateNavBadges = () => {
  ...
  const prospT = (state.prospectes || []).filter(p => !p.no_trucar && !['No interessa','Convertit'].includes(p.estat) && ((p.num_intents||0)===0 || (p.propera_accio_data && new Date(p.propera_accio_data) <= new Date())));
  set('nav-prospeccio', prospT.length);
