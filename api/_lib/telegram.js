...
// Envia un document (fitxer) al chat_id donat via multipart/form-data.
// Útil per a còpies de seguretat (JSON) i adjunts. Node 18+ (Vercel) té
// FormData, Blob i fetch globals.
export async function tgSendDocument(chatId, filename, content, caption) {
  if (!telegramConfigured()) return { ok: false, error: 'TELEGRAM_BOT_TOKEN no configurat' };
  if (!chatId) return { ok: false, error: 'chatId buit' };
  try {
    ...
    form.append('document', blob, filename || 'backup.json');
    const resp = await fetch(endpoint('sendDocument'), { method: 'POST', body: form });
    const data = await resp.json().catch(() => ({}));
